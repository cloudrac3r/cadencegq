...
module.exports = [
    ...
    {
        route: "/api/images", methods: ["POST"], upload: true, code: async ({body, url}) => {
            const params = url.searchParams
            if (!body.length) return [400, 4];
            if (!params.has("token")) return [401, 8];
            const account = await db.get("SELECT Accounts.userID, Accounts.canUpload FROM Accounts INNER JOIN AccountTokens USING (userID) WHERE AccountTokens.token = ?", [params.get("token")]);
    ...
    {
        route: "/api/images", methods: ["GET"], code: async () => {
